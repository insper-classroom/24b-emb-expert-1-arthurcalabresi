...
#define WIFI_SSID "AiPhone "
#define WIFI_PASSWORD "calamonstro"
...
void wifi_task(void *p) {
    ...
    adc_init();
    adc_gpio_init(26); // Configura o pino GP26 (ADC0) para o potenciômetro
    adc_select_input(0);
    // Contador
    int cnt = 0;
    ...
    while (1) {
        ...
        float tensao = (valor_adc * 3.3f) / (1 << 12); // Converte para tensão (0V a 3.3V)
        printf("Valor do potenciômetro (bruto): %d, Tensão: %.2f V\n", valor_adc, tensao);
        char payload_content[64];
        int payload_length = sprintf(payload_content, "dado=%f", tensao);
